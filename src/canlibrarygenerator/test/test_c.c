...
void run_can_test() {
    ...
    if (dbc_unpackage_message(can_message_id, can_message_data, message_length) == 0) {
        ...
        if (dbc_package_message(can_message_id) == 0) {
            printf("Packaging successful. Checking data integrity...\n");
            if (compare_data(can_message_data, msgMotor_01.base.data, msgMotor_01.base.dlc)) {
...
void run_canfd_test() {
    ...
    printf("Attempting to decode message with ID 0x%x...\n", canfd_canid);
    if (dbc_unpackage_message(canfd_canid, canfd_data, canfd_dlen) == 0) {
        ...
        if (are_equal(expected_canfd_values[0], msgVD_GNSS_precision_position.base.signals[0].value, 1E-6) == 0) {
            printf("  - ERROR: %s mismatch. Expected %f, got %f\n", msgVD_GNSS_precision_position.base.signals[0].name, expected_canfd_values[0], msgVD_GNSS_precision_position.base.signals[0].value);
            all_signals_ok = false;
        }
        if (are_equal(expected_canfd_values[1], msgVD_GNSS_precision_position.base.signals[1].value, 1E-6) == 0) {
            printf("  - ERROR: %s mismatch. Expected %f, got %f\n", msgVD_GNSS_precision_position.base.signals[1].name, expected_canfd_values[1], msgVD_GNSS_precision_position.base.signals[1].value);
            all_signals_ok = false;
        }
        if (are_equal(expected_canfd_values[2], msgVD_GNSS_precision_position.base.signals[2].value, 1E-6) == 0) {
            printf("  - ERROR: %s mismatch. Expected %f, got %f\n", msgVD_GNSS_precision_position.base.signals[2].name, expected_canfd_values[2], msgVD_GNSS_precision_position.base.signals[2].value);
            all_signals_ok = false;
        }
        ...
        // Test packaging process
        printf("\nVerifying packaging process...\n");
        if (dbc_package_message(canfd_canid) == 0) {
            printf("Packaging successful. Checking data integrity...\n");
            if (compare_data(canfd_data, msgVD_GNSS_precision_position.base.data, msgVD_GNSS_precision_position.base.dlc)) {
                printf("Data integrity test passed!\n");
            }
            else {
